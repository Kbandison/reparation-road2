...
// DELETE - delete one record, a set of records, or an entire collection
export async function DELETE(request: NextRequest) {
  const admin = await verifyAdmin();
  if (!admin) return NextResponse.json({ error: 'Forbidden' }, { status: 403 });

  const { tableName, id, ids, all, discriminatorColumn, discriminatorValue } =
    await request.json();
  ...
  // Bulk delete of an explicit set of ids — batched so the request URL stays
  // within PostgREST limits for large selections.
  if (Array.isArray(ids) && ids.length > 0) {
    for (let i = 0; i < ids.length; i += 100) {
      const chunk = ids.slice(i, i + 100);
  ...
  // Delete an entire collection. PostgREST refuses an unfiltered DELETE, so
  // scope by the discriminator when present, else an always-true filter
  // (id is never null) to cover the whole table.
  if (all) {
