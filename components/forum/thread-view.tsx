...
// The full thread (original post + replies + reply editor). Shared by the
// standalone thread page and the feed's intercepting-route modal.
export async function ThreadView({ slug, inModal = false }: { slug: string; inModal?: boolean }) {
  ...
  return (
    <>
      ...
      <div className="flex items-start justify-between gap-4 mb-2">
        <h1 className={`font-display font-semibold text-brand-cream ${inModal ? 'text-xl md:text-2xl' : 'text-2xl md:text-3xl'}`}>
          {thread.title}
        </h1>
      ...
      {/* Original post */}
      <div className="bg-brand-card border border-brand-gold/[0.08] rounded-2xl p-5 sm:p-6 mb-6">
      ...
      {posts && posts.length > 0 && (
        <div className="space-y-4 mb-8">
          ...
          {posts.map((post) => {
            ...
            return (
              <div key={post.id} className="bg-brand-card border border-brand-gold/[0.08] rounded-2xl p-5 sm:p-6">
                <div className="flex items-center gap-3 mb-4">
